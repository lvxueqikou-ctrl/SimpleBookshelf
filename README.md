...
将 `simplebookshelf.koplugin` 整个文件夹复制到：
...
```text
/mnt/us/koreader/plugins/simplebookshelf.koplugin/main.lua
```
...
当前版本不提供批量移动、批量删除、批量重命名，也不提供编辑书名、作者等书籍元数据的入口。书名主要来自书籍文件名或 KOReader 扫描到的书籍信息。
...
请根据项目实际使用的 KOReader 代码和第三方组件补充许可证。
